    ...
    PROCESS {
        ...
        foreach ($Media in $SelectedOSMedia) {
            ...
            if (Test-Path "$ISOSourceFolder\efi\microsoft\boot\efisys.bin") {
                $efisys = "$ISOSourceFolder\efi\microsoft\boot\efisys.bin"
            } elseif (Test-Path "$OSDBuilderContent\Tools\$env:PROCESSOR_ARCHITECTURE\Oscdimg\efisys.bin") {
                $efisys = "$OSDBuilderContent\Tools\$env:PROCESSOR_ARCHITECTURE\Oscdimg\efisys.bin"
