...
function New-OSDBuilderContentPack {
    ...
    foreach ($ContentDirectory in $ContentDirectories) {
        if (!(Test-Path (Join-Path $SetOSDBuilderPathContentPacks "$Name\$ContentDirectory"))) {
            New-Item (Join-Path $SetOSDBuilderPathContentPacks "$Name\$ContentDirectory") -ItemType Directory -Force | Out-Null
        }
    }
}